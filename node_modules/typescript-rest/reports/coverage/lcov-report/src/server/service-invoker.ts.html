...
<head>
    <title>Code coverage report for src/server/service-invoker.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/server</a> service-invoker.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">93.02% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>120/129</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>63/78</span>
            </div>
    ...
    public async callService(context: ServiceContext) {
        try {
            ...
            if (this.mustCallNext()) {
                context.next();
            } else <span class="missing-if-branch" title="if path not taken" >I</span>if (this.debugger.enabled) {
<span class="cstat-no" title="statement not covered" >                this.debugger('Ignoring next middlewares');</span>
            }
        }
        catch (err) {
            context.next(err);
...
&nbsp;
    private async callTargetEndPoint(context: ServiceContext) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.debugger.enabled) {
<span class="cstat-no" title="statement not covered" >            this.debugger('Invoking service method &lt;%s&gt; with params: %j', this.serviceMethod.name, args);</span>
        }
...
&nbsp;
    private getMethodToCall() {
        return this.serviceClass.targetClass.prototype[this.serviceMethod.name]
            || <span class="branch-1 cbranch-no" title="branch not covered" >this.serviceClass.targetClass[this.serviceMethod.name];</span>
    }
&nbsp;
...
&nbsp;
    private identifyAcceptedType(context: ServiceContext) {
        if (this.serviceMethod.resolvedAccepts) {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (accepts &amp;&amp; accepts.length &gt; 0) {
...
&nbsp;
    private processResponseHeaders(context: ServiceContext) {
        if (this.serviceMethod.resolvedLanguages) {
            <span class="missing-if-branch" title="else path not taken" >E</span>if (this.serviceMethod.httpMethod === HttpMethod.GET) {
...
<span class="cstat-no" title="statement not covered" >                    context.response.send(value.toString());</span>
<span class="cstat-no" title="statement not covered" >                    break;</span>
                ...
                case 'boolean':
                    ...
                    break;
                case 'undefined':
                    <span class="missing-if-branch" title="if path not taken" >I</span>if (!context.response.headersSent) {
<span class="cstat-no" title="statement not covered" >                        context.response.sendStatus(204);</span>
                    }
                    break;
                default:
                    value === null 
                        ? <span class="branch-0 cbranch-no" title="branch not covered" >context.response.send(value) </span>
...
&nbsp;
    private async sendComplexValue(context: ServiceContext, value: any) {
        ...
        else <span class="missing-if-branch" title="if path not taken" >I</span>if (value.then &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >value.catch)</span> {
            const val = <span class="cstat-no" title="statement not covered" >await value;</span>
<span class="cstat-no" title="statement not covered" >            await this.sendValue(val, context);</span>
        }
        else {
...
<span class="cstat-no" title="statement not covered" >            context.response.writeHead(200, {</span>
                'Content-Length': value.content.length,
                'Content-Type': value.mimeType
            });
        }
...
&nbsp;
    private downloadResToPromise(res: express.Response, value: DownloadResource) {
        ...
        return new Promise((resolve, reject) =&gt; {
            res.download(value.filePath, value.fileName || <span class="branch-1 cbranch-no" title="branch not covered" >value.filePath,</span> (err) =&gt; {
