...
export default function Aircrafts() {
  ...
  const [tableData, setTableData] = useState<Aircraft[]>(() => aircraftSampleList);
  const [validationErrors, setValidationErrors] = useState<{
    [cellId: string]: string;
  }>({});
  const [editRow, setEditRow] = useState<Aircraft | null>(null);
  const confirm = useConfirmDialog();

  const getCommonEditTextFieldProps = useCallback(
    (
      cell: MRT_Cell<Aircraft>,
    ): MRT_ColumnDef<Aircraft>['muiTableBodyCellEditTextFieldProps'] => {
      return {
  ...
  const columns = React.useMemo<MRT_ColumnDef<Aircraft>[]>(
    () => [
      {
        accessorKey: 'name',
  ...
  const handleCreateNewRow = (values: Aircraft) => {
    tableData.push(values);
    setTableData([...tableData]);
  };

  const handleSaveRowEdits: MaterialReactTableProps<Aircraft>['onEditingRowSave'] =
  ...
  const handleDeleteRow = (row: MRT_Row<Aircraft>) => {
    confirm({
  ...
  return (
    <>
      <Title>Список воздушных судов</Title>
      <TableContainer>
        <MaterialReactTable
      ...
      <EditRowDialog
        addTitle="Зарегистрировать воздушное судно"
        editTitle="Редактировать воздушное судно"
        columns={columns}
